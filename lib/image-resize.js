...
ImageResize.prototype.smartResizeDown = function (options) {
	...
	if ((!options.minWidth && !options.minHeight) || (options.minWidth && options.minHeight)) {
		...
		gm(image.gm.stream()).size(function (err, value) {
			if (!err) {
				if (value.height < options.minHeight) {
					if(image.format !== 'JPEG'){
						image.gm.background('transparency');
					}
					image.gm.resize(options.width, null).compose('Copy').gravity('Center').extent(options.width, options.minHeight);
				}
				deferred.resolve();
			}
			deferred.reject();
		...
		gm(image.gm.stream()).size(function (err, value) {
			if (!err) {
				if (value.width < options.minWidth) {
					...
					image.gm.resize(null, options.height).compose('Copy').gravity('Center').extent(options.minWidth, options.height);
				}
				deferred.resolve();
			}
			deferred.reject();
